...
import { initiateSignup } from '@/services/auth';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await initiateSignup({ email });
        ...
        router.push(`/auth/register/verify-otp?email=${encodeURIComponent(email)}`);
      } else {
        setError(response.message || 'Failed to start registration. Please try again.');
      }
    } catch (error) {
